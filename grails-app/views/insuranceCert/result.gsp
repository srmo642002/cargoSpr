...
<div id="list-InsuranceCert" ng-controller="InsuranceCertController" class="content scaffold-list" role="main">
    ...
    <br>
    <div id="tb">
        <div><table><tr><th>Coupon No</th><th>Purchase Date</th><th>Expire Date</th><th>Distribute Date</th><th>Customs</th><th>Shipment</th><th>Origin Custom</th><th>Destination Custom</th><th>Transit No</th><th>Commodity</th><th>Weight</th><th>Tariff</th><th>Row No</th><th>Kutazh No</th><th>Receipt Date</th><th>Receipt No</th></tr></table></div>
    <g:each in="${reportRes}" var="item">
        <div id="tb">
        <div><table><tr><td>${item.CouponNO}</td><td>${item.purchaseDate}</td><td>${item.expireDate}</td><td>${item.sendingDate}</td><td>${item.agent}</td><td>${item.shipment}</td><td>${item.transitFrom}</td><td>${item.transitTo}</td><td>${item.permitsNum}</td><td>${item.commodity}</td><td>${item.weight}</td><td>${item.tariff}</td><td>${item.rowNum}</td><td>${item.kutazhNum}</td><td>${item.receiptDate}</td><td>${item.receiptNum}</td></tr></table></div>
