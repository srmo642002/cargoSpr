...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'level', 'error')} required">
	<label for="level">
		<g:message code="user.level.label" default="Level" />
		<span class="required-indicator">*</span>
	</label>
	<g:select name="level" from="${userInstance.constraints.level.inList}" required="" value="${userInstance?.level}" valueMessagePrefix="user.level"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'groups', 'error')} ">
	...
	<g:select id="groups" name="groups.id" from="${cargo.UserGroup.list()}" optionKey="id" value="${userInstance?.groups?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
